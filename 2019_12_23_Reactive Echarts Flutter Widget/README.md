...
So we encapsulated a Flutter widget: [flutter_echarts](https://github.com/entronad/flutter_echarts) , which taking into account both extensibility and ease to use, helping Flutter developers to fully play the Echarts' functions.

# Feature

Before this, we encapsulated an [Echarts component](https://github.com/entronad/react-native-echarts-demo) in the React Native, and got some experiences about using data visualization libraries in a reactive UI framework, so combining with understandings about Flutter, we designed some features for flutter_echarts:

**Reactive Updating**

One of the most important features of Flutter is, like all other reactive UI frameworks, that it automatically update the view according to the change of data, which brings a lot of convenience for development. Echarts is independent of any UI frameworks, but it is designed driven by data and changes in data drive changes in the chart.

So we just need to connect the data driving of Echarts with the view updating of Flutter to implement the reactive updating. It's very simple to set dynamic data updating in Echarts. All data updating are through `setOption` . You only need to get data as you wish, fill in data to `setOption` without considering the changes brought by data, ECharts will find the difference between two group of data and present the difference through proper animation. In Flutter, when the container widget updates and the data props passed to the child widget changes, the  `State.didUpdateWidget` of this StatefulWidget will be triggered, and calling `setOption` in it could notify Echarts to update the chart. This process will make flutter_echarts used as easy as a simple StatelessWidget.

**Two Way Communication**

The communication between the chart and the outer logic is quite necessary. In flutter_echarts, the communication between JavaScript and Dart is just as that between father and child widgets: "props down, events up".

All settings and updatings from outside is passed to charts by  `option` and `extraScript` , and in the form of JavaScript code strings which executed by the WebView; On the other hand, the events inside the WebView are sent through JavascriptChannel and handled by the onMessage function. This is the two way communication between inside JavaScript and outside Dart.

**Configurable extensions**

Echarts has varies [extensions](https://echarts.apache.org/en/download-extension.html) , including charts, maps and WebGL. In the web ,we could import them as script files to extend the functions of Echarts. For out-of-box usage,  flutter_echarts embeded the newest version of Echarts script, no need of extra importing. Meanwhile, we export the `extensions` property for users to include any scripts needed.  `extensions` is a List of Strings, and users could directly copy the script strings in to the source code, which avoids file reading and complex asset dirs.

# Widget Properties

While encapsulating widgets, ease to use is often more importent than completeness. It should be able to use out-of-box for all levels of developers. Echarts it self is designed to the principle of ease, and try to put all configrations in `option` ([details in this paper](http://www.cad.zju.edu.cn/home/vagblog/VAG_Work/echarts.pdf)) . So flutter_echarts also simplified the widget properties:
...
These are all the 4 properties above, and else like when to update charts are made by inside mechanisms. This makes flutter_echarts looks just like a simple presentational StatelessWidget. Users just need to be famliar with Echarts and no additional learning.
...
We know that the loading of html in the WebView and the fetching of data are both asyc, and we do not which one will finish earlier. the order of lifecycles in the initialization of WebView is:

```
onWebViewCreated --> loading html --> onPageFinished
```

But WebViewController can only be reached in onWebViewCreated. In another word, when the widgetd get a WebViewController, we can not make sure that html has already loaded, so we can not tell it's ready to update in the `didUpdateWidget` by WebViewController.

Our solution is to decouple "data props changing causes chart updating" into two steps: "data props changing causes \_currentOption changing" and "updating of the chart according to \_currentOption", which makes sure that data fetched before the html is loaded can be record.
...
The webview_flutter provides a javascriptChannels property to set multiple named channels. But considering the users who are don't know about webview_flutter, flutter_echarts dose not expose this property. Instead, we build a channel named "Messager" inside:
...
If multiple types of events need to be sent, users can do as redux actions:
